...
SELECT
    ...
    pc.maintenance AS maintenance, -- Fixed typo: maintanance -> maintenance
    pn.prd_cost AS cost,
...
LEFT JOIN silver_erp_px_cat_g1v2 pc
    ON pn.cat_id = pc.id
WHERE pn.prd_end_dt IS NULL; -- Filters out historical versions (keeps active products only)
...
-- Connecting to Gold views to fetch Surrogate Keys (Star Schema mapping)
